...
const channelSchema = new mongoose.Schema({
  channelId: { 
    ...
    required: true, 
    unique: true 
  }, // Numeric Telegram ID (e.g. "123456789") or handle
  username: { 
    type: String, 
    required: true 
  },
  ...
  isActive: { 
    type: Boolean, 
    default: false // Disabled by default when synced from Telegram
  },
  country: {
    type: String,
    default: 'IN'
  ...
  // per-message AI classifier in verifier.js; any other value is a Master-managed category
  // slug. No hardcoded enum on purpose — a stale list here would silently block categories
  // added to Master later (the same bug OutputChannel.category and Deal.category were fixed for).
  category: {
    type: String,
    default: 'auto'
  },
...
const Channel = mongoose.model('Channel', channelSchema, 'monitored_channels');
